...
> **STATUS (2026-05-22)**: 本文档写于 Paper 1 实验启动**之前**，记录的是当时的代码状态。
> 多数 P0/P1 已经在后续 commit 中修复（包括 1.4 第 4 点提到的 cross-ISA mnemonic bug）。
> 当前生效状态见文末 **§Update history (2026-05-22)**；本文正文保留原貌，
> 不要把"x86 branch_constraints 偏弱"误当成 paper 当前的 limitation。
...
## Update history (2026-05-22)

回头审计的时候 fact-check 了一遍：本文 §1.4 第 4 点和 §Part 3 item #9 提到
的 "x86 branch_constraints 偏弱" 这个 bug **已经在后续 commit 中被修了**，但本文
没及时更新，导致 Paper 1 起草时一度被当成 still-existing limitation 写进 §9。
以下是当前事实状态（以 HEAD `85223d8` 为基准）：

### Ghidra extractor (✅ 已修)

`scripts/ghidra/FuzzPilotGhidraExtract.java` 当前用 **prefix match** 处理 cmp 类
指令（line 189-194），同时覆盖 x86 和 ARM 两套助记符。注释明确写着 "Match the
shared prefixes so x86 binaries don't end up with empty branch_constraints."
`isConditionalBranch` (line 207-213) 也同时覆盖 `j*` (x86) 和 `b.*` / `cbz`
/ `cbnz` / `tbz` / `tbnz` (ARM)。**§1.4 第 4 点描述的 "x86 上 branch_constraints
数量偏少" 在当前代码不成立**。Paper 1 §9 不再把这个写成 limitation。

### Paper 1 P0 阻塞项 (全部 ✅)

| audit Part 3 item | 当前状态 |
|---|---|
| #1 cjson `AFL_MAP_SIZE` → 65536 | ✅ `experiments/targets/cjson/config.yaml:13` |
| #2 libpng `AFL_MAP_SIZE` → 131072 | ✅ `experiments/targets/libpng/config.yaml:13` |
| #3 cjson seeds ≥ 20 | ✅ 实际 26 个 |
| #4 libpng seeds ≥ 20 | ✅ 实际 21 个 |
| #5 custom_mutator_path 修 | ✅ Linux host 上 `./build/...` 解析正确（preflight passed） |
| #6 preflight 加 ghidra/java/mutator-arch 检查 | ✅ `scripts/paper01/preflight.sh` 已包含 |
| #7 Paper 1 加 `no-static-analysis` ablation | ✅ `experiments/manifests/paper01_preprint.yaml` 含 E2c |
| #8 docker 装 JDK + Ghidra | ✅ `docker/ubuntu/Dockerfile` 已包含 openjdk-17 + Ghidra 11.1.2 |
| #9 Ghidra extractor 修 x86 mnemonic | ✅ 见上文 |

### Paper 1 文档勘误 (§Part 4)

| Part 4 item | 当前状态 |
|---|---|
| #1 §3 三大支柱补 "static analysis = LLM prompt context" | ✅ `papers/paper01/paper01.tex` §3/§5 已 framing 准确，并指明 dual-consumer (blackboard + AFL dict) |
| #2 §6 加 static-analysis RQ | ✅ E2c 作为 RQ3 的 ablation 行 |
| #3 实验矩阵加 E2c × 3 reps × 4h | ✅ `paper01_preprint.yaml` |
| #4 manifest 加 E2c | ✅ 同上 |
| #5 preflight 加 Ghidra/Java/mutator ELF | ✅ |
| #6 三个 target config 改 AFL_MAP_SIZE/mutator path | ✅ |
| #7 新增 cjson seeds | ✅ 26 个 |

### 当前实际的 Paper 1 limitation（替代本文 §1.4 的过时清单）

实测 (HEAD `85223d8`) 仍存在的、需要写进 paper §9 的 limitation：

- Ghidra 在 open-source target 上未充分 demo binary-only 场景的特有优势 ——
  open-source 目标 LLVM-IR / angr 理论上能给同样字段；binary-only target
  评估留给 venue 版。
- Branch constraint 抽取依赖紧邻 cmp 的 4 条指令窗口；间隔被多条算术指令
  的条件分支会丢，这不是 cross-ISA bias，是 window depth 上限。
- single target family (cjson)；libpng 是次要 data point。
- N=3-5 reps，没有 formal significance test。
- LLM 非确定性（temperature=0 也不保证 bit-exact，靠 audit log 保证 replay）。

### 这次 audit 给 paper 写作的教训

不要直接引用 audit 文档里写"未修"的 limitation 到 paper 正文 —— 即使是自己一周
前写的 audit。每次 paper 引用任何 audit 结论之前，先 grep 当前代码确认状态。
本次差点把一个早已修好的 bug 当成 known limitation 写进 §9。

---

*Last updated: 2026-05-22 — verified against HEAD `85223d8`.*
