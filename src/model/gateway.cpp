...
std::filesystem::path make_private_tempfile(const std::string& prefix,
  ...
  // RAII guard for the fd and path
  struct TempFileFdGuard {
    int fd;
    char* path;
    bool release = false;
    ~TempFileFdGuard() {
  ...
  if (close(fd) != 0) {
    guard.fd = -1; // already closed (attempted), but failed, so unlink
    return {};
  }
  guard.fd = -1; // successfully closed

  // Construct path - this can throw! If it does, guard cleans up the file.
  std::filesystem::path result(buf.data());

  guard.release = true; // Everything succeeded, we want to keep the file.
  return result;
}
