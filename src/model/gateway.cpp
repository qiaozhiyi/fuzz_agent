...
OpenAICompatibleGateway::complete_json(const ModelRequest &request) {
  ...
  const std::string escaped_system_prompt = json_escape(request.system_prompt);
  const std::string escaped_user_context =
      json_escape(request.user_context_json);

  std::string payload_str;
  payload_str.reserve(512 + model_.size() + escaped_system_prompt.size() +
                      escaped_user_context.size());
  ...
  payload_str += escaped_system_prompt;
  payload_str += "\"},{\"role\":\"user\",\"content\":\"";
  payload_str += escaped_user_context;
  payload_str +=
      "\"}],\"response_format\":{\"type\":\"json_object\"},\"max_tokens\":";
  payload_str += std::to_string(request.max_output_tokens);
  payload_str += ",\"temperature\":";
  payload_str += std::to_string(request.temperature);
  payload_str += ",\"top_p\":";
  payload_str += std::to_string(request.top_p);
  if (request.seed != 0) {
    payload_str += ",\"seed\":";
...
ModelResponse GeminiGateway::complete_json(const ModelRequest &request) {
  ...
  std::string payload_str;
  payload_str.reserve(256 + escaped_text.size());

  payload_str += "{\"contents\":[{\"role\":\"user\",\"parts\":[{\"text\":\"";
  payload_str += escaped_text;
  payload_str += "\"}]}],\"generationConfig\":{\"temperature\":";
  payload_str += std::to_string(request.temperature);
  payload_str += ",\"topP\":";
  payload_str += std::to_string(request.top_p);
  payload_str += ",\"maxOutputTokens\":";
  payload_str += std::to_string(request.max_output_tokens);
  payload_str += ",\"responseMimeType\":\"application/json\"}}";
  response.full_request_payload = payload_str;
  ...
  const std::string url =
      endpoint_ + "/" + model_ + ":generateContent?key=" + std::string(api_key);
  const std::string max_time =
      std::to_string(std::max<uint32_t>(1, request.timeout_ms / 1000));
