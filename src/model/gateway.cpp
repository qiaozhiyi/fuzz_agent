...
extract_content_field(const std::string &response) {
...
// Extract content from OpenAI-compatible response, with fallback to
// reasoning_content. GLM-4.5-flash and GLM-4.7-flash return JSON in
// "reasoning_content" field when using response_format: json_object, while
// "content" may be empty or incomplete. This function tries "content" first,
// then falls back to "reasoning_content" if content is empty or doesn't contain
// valid JSON (no opening brace).
std::string
extract_openai_content_with_reasoning_fallback(const std::string &response) {
  // Try standard "content" field first
  const std::string content_needle = "\"content\":\"";
  auto pos = response.find(content_needle);
  if (pos != std::string::npos) {
    std::string content =
        decode_json_string_at(response, pos + content_needle.size());
    ...
    while (first_non_space < content.size() &&
           (content[first_non_space] == ' ' ||
            content[first_non_space] == '\n' ||
            content[first_non_space] == '\r' ||
            content[first_non_space] == '\t')) {
      ++first_non_space;
    }
...
[[maybe_unused]] bool write_private_text_file(const std::filesystem::path &path,
  ...
  // Security: Use RAII to prevent fd leaks on exception during write
  // operations.
  struct FdGuard {
    int fd;
    ~FdGuard() {
  ...
  while (written < content.size()) {
    const ssize_t n =
        write(guard.fd, content.data() + written, content.size() - written);
    if (n < 0) {
      if (errno == EINTR) {
        continue;
      }
  ...
  int close_result = close(guard.fd);
  guard.fd = -1; // Prevent double-close
...
std::filesystem::path make_private_tempfile(const std::string &prefix,
  ...
  // Security: Use RAII to ensure file descriptors and temporary files are
  // safely cleaned up even if subsequent operations throw exceptions.
  struct TempGuard {
    int fd;
    const char *path;
    bool release = false;
  ...
  (void)fchmod(guard.fd, S_IRUSR | S_IWUSR);
  std::size_t written = 0;
  while (written < content.size()) {
    const ssize_t n =
        write(guard.fd, content.data() + written, content.size() - written);
  ...
  guard.fd = -1; // Mark as closed

  std::filesystem::path result(buf.data());
  guard.release = true; // Path construction succeeded, transfer ownership
  return result;
}
...
// Coarse error classification based on curl exit + raw body keywords + HTTP
// status.
std::string classify_error(const ProcessCaptureResult &curl,
                           const std::string &raw, int http_status) {
  if (!curl.spawned)
    return "spawn_error";
  if (curl.signaled)
    return "signaled";
  ...
  const auto http_error =
      classify_openai_compatible_http_error(raw, http_status);
  if (http_error != "ok") {
    return http_error;
  }
  return "ok";
...
std::string classify_openai_compatible_http_error(const std::string &raw,
                                                  int http_status) {
...
OpenAICompatibleGateway::OpenAICompatibleGateway(std::string endpoint,
    ...
    : endpoint_(std::move(endpoint)), model_(std::move(model)),
      api_key_env_(std::move(api_key_env)), disable_thinking_(disable_thinking),
...
OpenAICompatibleGateway::complete_json(const ModelRequest &request) {
  ...
  // Rate limiting for free-tier APIs: enforce 1-second minimum interval between
  // requests to avoid overwhelming GLM-4.7-flash and similar free models
  // (typically 1-2 req/sec limit). Reserve the next request slot BEFORE
  // releasing the lock to prevent race conditions.
  {
  ...
  const std::vector<std::string> argv = {
      ...
      max_time,
      "-w",
      "\nHTTP_STATUS:%{http_code}", // Extract HTTP status for better error
                                    // classification
      "-H",
  ...
  // Exponential backoff retry for transient errors and rate limiting (HTTP
  // 429/503)
  constexpr int max_retries = 2;
  const int backoff_delays_ms[] = {1000, 2000};

  for (int retry = 0;
       retry < max_retries && (is_transient_transport_error(curl, raw) ||
                               http_status == 429 || http_status == 503);
       ++retry) {
    ++response.retry_count;
    std::this_thread::sleep_for(
        std::chrono::milliseconds(backoff_delays_ms[retry]));
