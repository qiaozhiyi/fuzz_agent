...
std::string afl_stats_json(const AflStats& stats) {
  // Performance optimization: Avoid std::ostringstream overhead by pre-allocating
  // a std::string buffer and using direct concatenation (+=).
  std::string out;
  out.reserve(512);
  out += "{\"ts\":";
  ...
  out += ",\"execs_per_sec\":";
  char buf[64];
  std::snprintf(buf, sizeof(buf), "%g", stats.execs_per_sec);
  out += buf;
  out += ",\"paths_total\":";
  ...
  out += ",\"bitmap_cvg\":";
  std::snprintf(buf, sizeof(buf), "%g", stats.bitmap_cvg);
  out += buf;
  out += ",\"stability\":";
  std::snprintf(buf, sizeof(buf), "%g", stats.stability);
  out += buf;
  out += ",\"last_path\":";
