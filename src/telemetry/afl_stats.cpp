...
std::string afl_stats_json(const AflStats &stats) {
  ...
  out += "{";
  out += "\"ts\":" + std::to_string(stats.sampled_at) + ",";
  out += "\"execs_done\":" + std::to_string(stats.execs_done) + ",";
  out += "\"execs_per_sec\":" + format_double(stats.execs_per_sec) + ",";
  out += "\"paths_total\":" + std::to_string(stats.paths_total) + ",";
  out += "\"edges_found\":" + std::to_string(stats.edges_found) + ",";
  out += "\"stale\":";
  out += (stats.stale ? "true" : "false");
  out += ",";
  out += "\"unique_crashes\":" + std::to_string(stats.unique_crashes) + ",";
  out += "\"unique_hangs\":" + std::to_string(stats.unique_hangs) + ",";
  out += "\"bitmap_cvg\":" + format_double(stats.bitmap_cvg) + ",";
  out += "\"stability\":" + format_double(stats.stability) + ",";
  out += "\"last_path\":" + std::to_string(stats.last_path) + ",";
  out += "\"recipe_hits\":" + std::to_string(stats.recipe_hits) + ",";
  out += "\"recipe_misses\":" + std::to_string(stats.recipe_misses) + ",";
  out += "\"raw\":{";
  bool first = true;
...
std::string afl_stats_summary(const AflStats &stats) {
  std::string out;
  out.reserve(256); // Pre-allocate to reduce reallocations
  out += "execs_done=" + std::to_string(stats.execs_done);
  out += " execs_per_sec=" + format_double(stats.execs_per_sec);
  out += " paths_total=" + std::to_string(stats.paths_total);
  out += " edges_found=" + std::to_string(stats.edges_found);
